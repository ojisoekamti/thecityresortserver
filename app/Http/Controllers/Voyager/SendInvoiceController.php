...
class SendInvoiceController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    {
		if(isset($request['send_invoice_hasmany_unit_relationship'])){
			foreach($request['send_invoice_hasmany_unit_relationship'] as $row){
				$getTenant = DB::select("SELECT *,tenants.isinactive as active FROM tenants INNER JOIN units ON units.id = tenants.id_unit WHERE tenants.id_unit = $row");
				
				foreach ($getTenant as $data) {
					if($data->active!="1"){
						continue;
					}
					$send = [
        ...
        if ($request->hasFile('image')) {
            ...
            foreach ($files as $file) {
                array_push($fileName, $file->store('/public/blast-messages'));
            }
        }

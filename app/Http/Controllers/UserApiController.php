    ...
    public function getUserDelegate(Request $request)
    {
        ...
        if ($uid != "") {
            ...
            if ($user_role[0]->lev2 > 0 && $user_role[0]->lev3 > 0) {
                // dump("dansek");
                $get_user = DB::select("SELECT DISTINCT * FROM role_group LEFT JOIN user_roles ON user_roles.user_id = role_group.id WHERE lev2 > 0 AND lev3 > 0 AND role_group.role_id = " . $role . " AND id != " . $uid . " AND user_roles.role_id != " . $user[0]->role_id);
